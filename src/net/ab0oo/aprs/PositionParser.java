 ...
 * including compressed, uncompressed, and MicE encoded positions 
 */
...
public class PositionParser {
	...
	public static Position parseMICe(byte[] msgBody, final String destinationCall)
		...
		if (dcall.length() != 6) {
			throw new UnparsablePositionException(
					"MicE Destination Call incorrect length:  " + dcall);
		}
		...
		for (int i = 0; i < 3; ++i) {
			...
			if (!(('0' <= c && c <= '9') || ('A' <= c && c <= 'L') || ('P' <= c && c <= 'Z'))) {
				throw new UnparsablePositionException("Digit " + i
						+ " dorked:  " + c);
			}
		}
		for (int i = 3; i < 5; ++i) {
		...
		if (c < '\u0026' || c > '\u007f') {
			throw new UnparsablePositionException("Raw packet contains " + c
					+ " at position " + 1);
		}
		c = (char) msgBody[1 + 1];
		if (c < '\u0026' || c > '\u0061') {
		...
		try {
			...
			throw new UnparsablePositionException(
					"Destination Call invalid for MicE:  "
							+ new String(destcall2));
		}
		...
		case 0:
			...
			break;
		case 1:
			lng = ((float) longDeg + ((float) longMin) / 60.0F + ((float) (longMinFract
					- longMinFract % 10 + 5) / 6000.0F));
			break;
		case 2:
		...
		default:
			throw new UnparsablePositionException(
					"Unable to extract longitude from MicE");
		}
	...
	public static Position parseNMEA(byte[] msgBody) throws Exception {
		...
		if (nmea.length < 5) { // Too few parts
			throw new UnparsablePositionException(
					"Too few parts in NMEA sentence");
		}
		...
		} else if (nmea.length > 15 && "$PNTS".equals(nmea[0])
				&& "1".equals(nmea[1])) { // PNTS version 1
		...
		} else if ("$GPGSA".equals(nmea[0]) || "$GPVTG".equals(nmea[0])
				|| "$GPGSV".equals(nmea[0])) {
	...
	public static Position parseCompressed(byte[] msgBody, int cursor) throws Exception {
		...
		for (int i = 1; i < 9; ++i) {
			...
			if (c < 0x21 || c > 0x7b) {
				throw new UnparsablePositionException(
						"Compressed position characters out of range");
			}
		}
		...
		float lat = 90.0F - ((float) (lat1 * 91 * 91 * 91 + lat2 * 91 * 91
				+ lat3 * 91 + lat4) / 380926.0F);
		float lng = -180.0F
	...
	/*
	 * private boolean validSymTableCompressed(char c) { if (c == '/' || c ==
